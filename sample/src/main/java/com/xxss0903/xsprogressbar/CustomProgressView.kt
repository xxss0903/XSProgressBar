...
class CustomProgressView @JvmOverloads constructor(
    ...
    private val timeSpace:Long
        get() {
            return progressTotal / DEFAULT_REPEATE_TIMES
        }
    private val progressSpace: Long
        get() {
            return progressTotal / DEFAULT_REPEATE_TIMES
        }
            ...
            PROGRESS_TYPE_LINE -> {
                when (textPosition) {
                    TEXT_POSITION_TOP -> {
                        ...
                        progressRect.top = (textRect.top + progressMargin)
                ...
                val progressLength = measuredWidth - progressRect.left * 2
                rectRight = (progressRect.left + (progressLength - indicatorSize) * 1)
                progressBgRect =
                    RectF(progressRect.left, progressRect.top, progressRect.left + progressLength, progressRect.bottom)
            }
        }
        if (drawIndicator) {
